...
# check if the system tables where already created, in which cases the directory /var/lib/mysql/mysql will exist. Ifit does not exist, install the system tables and store the data in /var/lib/mysql
...
DROP DATABASE test;
DELETE FROM mysql.db WHERE Db='test' or 'test\_%';
